...
export const TagInput = ({ props }: { props: ITagInputProps }): ReactElement => {
  const {
    values,
    projectTagOptions,
    setFieldValue
    ...
    >
      <div className={`border-[1px] border-primary-border min-h-12 rounded-xl overflow-hidden relative flex items-center w-full text-sm tracking-wide bg-[#F8F8F8]`}>
        <div className="h-full flex gap-2 py-3 items-center justify-start px-3 flex-wrap w-full">
          {values.map((value, index) => (
            ...
            >
              ...
              <button
                type="button"
                className="text-sm"
                onClick={() => { setFieldValue('tags', values.filter((item) => item !== value))}}
              >
          ...
          {
            values.length <= 0 && <span className="tetx-gray-200 text-xs">Choose or create tag</span>
          }
        </div>
      ...
      <div className={`absolute z-20 right-0 ${isTagSelectFieldFocus ? '' : 'hidden'} w-full`}>
        <div className="mt-2 rounded-xl top-full max-h-40 grid grid-cols-2 gap-3 overflow-y-auto bg-white shadow-xl p-2 border">
          {
            projectTagOptions.filter((tag) => tag !== '').map((tag, index) =>
              <button
                type="button"
                key={index}
                onClick={() => {
                ...
                title={tag}
                className={`rounded-xl border px-4 py-2 text-sm ${values.includes(tag) ? 'bg-gray-400 text-white' : 'bg-white hover:bg-gray-200'} disabled:bg-gray-300 disabled:text-white text-ellipsis overflow-x-hidden`}
      ...
      <div className={`absolute z-20 right-0 ${isTagInputFieldFocus ? '' : 'hidden'} w-full`}>
        <div className="mt-2 rounded-xl top-full max-h-40 flex gap-3 overflow-y-auto bg-white shadow-xl py-2 px-3 border">
          ...
          />
          {
            !values.includes(newTagInput)
              ...
              : <span className="text-xs text-red-400 ml-auto w-fit grow line-clamp-1 my-auto">Already exist</span>
          }
        </div>
      </div>
    </div>
