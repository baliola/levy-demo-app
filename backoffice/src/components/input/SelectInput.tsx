...
interface ISelectInputProps {
  ...
  ) => Promise<void | FormikErrors<IAccountValues>>;
}
...
export const SelectInput = ({
  ...
  const { value, options, field, setFieldValue } = props;
        ...
        <div className="mt-2 rounded-xl top-full max-h-40 grid gap-3 overflow-y-auto bg-white shadow-xl p-2 border">
          {options.map((option, index) => (
            <button
              ...
              title={option.name}
              className={`capitalize rounded-xl border px-4 py-2 text-xs ${
                option === value
